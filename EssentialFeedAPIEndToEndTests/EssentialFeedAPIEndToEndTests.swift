...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
	...
	func test_endToEndTestServerGETFeedResult_matchesFixedAccountData() {
		...
		case .success(let imageFeed):
			XCTAssertEqual(imageFeed.count, 8, "Expected 8 imageFeed in the test account feed")
		...
		case .failure(let error):
			XCTFail("Expected a successful feed result but got error \(error)")
		default:
	...
	private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> FeedLoader.Result? {
		let testServerURL: URL = URL(string: "https://essentialdeveloper.com/feed-case-study/test-api/feed")!
		let client: URLSessionHTTPClient = URLSessionHTTPClient(session: URLSession(configuration: .ephemeral))
